...
function Project({ title, image, description, techstack, previewLink, githubLink }) {
  return (
    <article className="rounded-xl mt-10 overflow-hidden shadow-xl shadow-slate-900 bg-dark-card">
      <img src={image} alt="" loading="lazy" className="max-w-full h-auto max-h-60 object-cover w-full" />
      <div className=" p-4">
        <h1 className="text-light-heading font-semibold text-lg pt-1">{title}</h1>
        <p className=" pt-4 text-light-content">{description}</p>
